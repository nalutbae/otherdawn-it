...
export function Hero() {
  ...
  const scale = useSpring(useTransform(scrollY, [0, 500], [1, 0.8]), {
    stiffness: 100,
    damping: 30,
  });
    ...
    <section className="relative flex min-h-screen items-center justify-center overflow-hidden px-4 pt-16">
      ...
      >
        <m.div 
          ...
          style={{ y: y1, x: useTransform(scrollY, [0, 500], [0, mousePosition.x * 2]) }}
        />
        <m.div 
          className="absolute right-1/4 top-1/2 h-[400px] w-[400px] rounded-full bg-accent-cyan/5 blur-[100px]"
          style={{ y: y2, x: useTransform(scrollY, [0, 500], [0, -mousePosition.x]) }}
        />
        <m.div 
